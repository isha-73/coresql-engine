...
void Parser::expect(TokenType type, const std::string& value) {
    if (!match(type, value)) {
        std::string expected = value.empty() ? "token type " + std::to_string((int)type) : value;
        throw std::runtime_error("Syntax error: expected " + expected + " but found " + peek().value);
    }
}
...
std::shared_ptr<Query> Parser::parse_insert() {
    ...
    while (!match(TokenType::SYMBOL, ")")) {
        ...
        if (val_token.type == TokenType::STRING || val_token.type == TokenType::INTEGER) {
            query->values.push_back(val_token.value);
        } else {
            throw std::runtime_error("Syntax error: expected value (string or integer)");
        }
...
std::shared_ptr<Query> Parser::parse_select() {
    ...
    if (col_token.type == TokenType::IDENTIFIER) {
        query->columns.push_back(col_token.value);
    } else if (col_token.type == TokenType::OPERATOR && col_token.value == "*") {
    ...
    if (match(TokenType::KEYWORD, "WHERE")) {
        ...
        if (where_col.type != TokenType::IDENTIFIER) throw std::runtime_error("Syntax error: expected column in WHERE");
        query->where_clause.column = where_col.value;
        
        const Token& op_token = advance();
        if (op_token.type != TokenType::OPERATOR) throw std::runtime_error("Syntax error: expected operator in WHERE");
